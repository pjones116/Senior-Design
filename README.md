...
•	The following is a list and description of all Python packages needed to run this application:
  ...
  -	time – Used to implement wait periods, timings
  
  -	sys – Used to force clean exit from script on error outs
  
  -	OS – Used to execute CMD commands from inside script
  
  -	subprocess – Used to call CMD subprocesses from inside script (i.e. rsync calls)
  
  -	picamera – Used to operate and manipulate Pi camera
  
  -	datetime – Used to establish time stamped naming conventions
  
  -	RPi.GPIO – Used to operate Pi GPIO ports
  
  -	Adafruit_DHT – Used to gather temperature and humidity data from Adafruit DHT11 or DHT22 sensor
  
  -	IO – Used to create and implement filestreams
  
  -	fcntl – Used to interact with I2C parameters for Atlas sensors
  
  -	string – Used to interact with and manipulate strings
  
  -	copy – Used to move I2C parameters between addresses by copying values under certain specified conditions
  
  -	RQ – Used to simply implement a Redis job queue (note: this is unofficial)
  
  -	Redis – Used to connect to Redis queue from inside script (note: this is official)
  
  -	Pyodc – Used to setup script connection to SQL Server 
...
•	atlas.py:
  ...
  -	Takes sensor readings and images, submits data to Redis queue and uploads CSV and images to directory on data server
  
  -	Run at user’s discretion (i.e. run when user wants to run an experiment)
  
  -	Execute with the following: “python atlas.py”

•	atlas_worker.py:

  -	Acts as middleman and takes enqueued data to be submitted to queue and pushed to the correct script
  
  -	This script can be left running at all times due to low CPU and memory impact, as it will process results any time they are submitted
  
  -	Execute with the following: “python atlas_worker.py <queue_name>”

•	atlas_upload.py:

  -	Uploads the queued data to the remote data server’s SQL Server table
  
  -	NEVER interacted with by the user, this script is only used when called from the main atlas.py

## __System Operational Notes__:
•	Python version to be used – 2.7.13

  -	Though Python 2.7 is going to be considered fully deprecated soon, the only way that the current build works with Atlas’ provided I2C software is if it is used
  
  -	NOTE: ensure that the relevant version of Pip is deployed for installing packages for the correct version of Python

•	Constantly running scripts

  -	The easiest way to implement the technologies discussed in this guide without too much maintenance is to have several scripts     continually run (or to add them to your Bash configuration for launch on startup)
  
  -	It is recommended that both Pagekite addresses (main and VNC), atlas_upload, and redis-server stay continually running
